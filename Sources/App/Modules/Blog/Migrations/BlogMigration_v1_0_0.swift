...
// swift run App serve --auto-migrate：如果您希望在运行其他命令之前自动运行迁移，则可以传递该--auto-migrate标志
// swift run App migrate --revert：要撤消数据库上的迁移，请migrate使用该--revert标志运行
    ...
    func prepare(on database: Database) -> EventLoopFuture<Void> {
        database.eventLoop.flatten([
            ...
            database.schema(BlogPostModel.schema)
                ...
                .field(BlogPostModel.FieldKeys.imageKey, .string, .custom(""))
